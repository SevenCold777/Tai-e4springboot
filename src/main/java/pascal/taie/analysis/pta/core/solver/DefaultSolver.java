...
public class DefaultSolver implements Solver {
        ...
        while (!workList.isEmpty() && !isTimeout) {
            ...
            while (!workList.isEmpty() && !isTimeout) {
                ...
                if (entry instanceof WorkList.PointerEntry pEntry) {
                    ...
                    if (p.toString().equals("[]:<org.joychou.controller.SQLI: java.lang.String jdbc_sqli_vul(java.lang.String)>/$r5")){
                        int a = 1;
                    }
    ...
    private PointsToSet propagate(Pointer pointer, PointsToSet pointsToSet) {
        ...
        for (CSObj o : pointsToSet){
            if (o.toString().contains("Taint") && pointer.toString().contains("toLowerCase"))
                continue;
            pts.addObject(o);
        }
        for (CSObj o : pts){
            if (o.toString().contains("Taint") && o.toString().contains("<org.owasp.webgoat.lessons.sqlinjection.mitigation.SqlInjectionLesson10a: org.owasp.webgoat.container.assignments.AttackResult completed(java.lang.String,java.lang.String,java.lang.String,java.lang.String,java.lang.String,java.lang.String,java.lang.String)>"))
                System.out.println("Propagate" + o.toString() + "to" + pointer.toString());
        }
        ...
        PointsToSet diff = getPointsToSetOf(pointer).addAllDiff(pts);
        if (!diff.isEmpty()) {
    ...
    private class StmtProcessor {
        ...
        private class Visitor implements StmtVisitor<Void> {
            ...
            public Void visit(Invoke stmt) {
                ...
                if (stmt.isDynamic()){
                    if (stmt.getInvokeExp().toString().contains("makeConcatWithConstants")){
                        Var res = stmt.getLValue();
                        for (Var v : stmt.getInvokeExp().getArgs()){
                            addPFGEdge(csManager.getCSVar(context,v),csManager.getCSVar(context,res),FlowKind.OTHER);
                        }
                    }
                }
                return null;
